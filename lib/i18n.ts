import type { Language } from "@/lib/types";

export const DEFAULT_LANGUAGE: Language = "en";

export const LANGUAGE_NAMES: Record<Language, string> = {
  bg: "Български",
  en: "English",
  de: "Deutsch",
};

type TranslationValue = string | TranslationTree;
type TranslationTree = {
  [key: string]: TranslationValue;
};

export const TRANSLATIONS: Record<Language, TranslationTree> = {
  en: {
    language: {
      label: "Language",
      change: "Change language",
      english: "English",
      bulgarian: "Bulgarian",
      german: "German",
    },
    nav: {
      dashboard: "Dashboard",
      ask: "Ask",
      browse: "Browse",
      history: "History",
    },
    common: {
      loading: "Loading...",
      close: "Close",
      clear: "Clear",
      search: "Search",
      email: "Email",
      password: "Password",
      fullName: "Full Name",
      noResults: "No results found",
      tryAgain: "Try again",
      getStarted: "Get started",
      createAccount: "Create Account",
      logIn: "Log in",
      signUp: "Sign up",
      viewAll: "View all",
      all: "All",
      questions: "Questions",
      documents: "Documents",
      completed: "Completed",
      inProgress: "In Progress",
      waiting: "Waiting",
      submitted: "Submitted",
      pending: "Pending",
      missing: "Missing",
    },
    landing: {
      trustBadges: {
        secure: "Secure & Private",
        instant: "Instant Analysis",
        users: "10k+ Users",
      },
      hero: {
        badge: "AI-powered guidance for any process",
        titlePrefix: "Navigate bureaucracy",
        titleHighlight: "without the headache",
        subtitle:
          "Upload documents, ask questions, and get step-by-step guidance through any administrative process. From visa applications to business permits.",
        tryFree: "Try it free",
        seeHow: "See how it works",
        trust: "No credit card required. 1 free analysis to get started.",
        docLabel: "Document",
        docValue: "Analyzed",
        stepLabel: "Step 3 of 5",
        stepValue: "Completed",
      },
      demo: {
        title: "Try it now",
        subtitle:
          "Ask any question about bureaucratic procedures and see how FormWise can help.",
        placeholder: "Ask about any bureaucratic procedure...",
        tryLabel: "Try:",
        ctaText: "Want full access? Create a free account to track your processes.",
        ctaButton: "Create Account",
        samples: {
          residence: "How do I apply for a residence permit in Bulgaria?",
          visa: "What documents do I need for a work visa?",
          company: "How to register a company abroad?",
        },
      },
      features: {
        title: "Everything you need to conquer paperwork",
        subtitle:
          "Powerful features designed to make bureaucratic processes simple and stress-free.",
        items: {
          analysisTitle: "Document Analysis",
          analysisDescription:
            "Upload any document and get instant insights. Our AI understands forms, applications, and official letters.",
          guidanceTitle: "Step-by-Step Guidance",
          guidanceDescription:
            "Get clear, numbered steps for any procedure. Know exactly what to do next and never miss a requirement.",
          supportTitle: "Multi-Country Support",
          supportDescription:
            "Whether you are abroad or at home, get localized guidance for procedures across different countries.",
          timeTitle: "Time Estimates",
          timeDescription:
            "Know how long each step takes. Plan your time and avoid unnecessary trips or delays.",
          checklistTitle: "Document Checklist",
          checklistDescription:
            "Interactive checklists ensure you have everything ready. Track your progress and tick off completed items.",
          askTitle: "Ask Anything",
          askDescription:
            "Have a specific question? Just ask. Get instant, accurate answers about any bureaucratic process.",
        },
      },
      authPrompt: {
        title: "Create a free account",
        body:
          "You've used your free trial. Create an account to continue using FormWise and track your bureaucratic processes.",
        later: "Maybe later",
        create: "Create Account",
        existing: "Already have an account? Log in",
      },
      footer: {
        product: "FormWise - AI-powered bureaucracy navigation",
        tagline: "Built for those navigating complex processes",
      },
      loading: "Analyzing your question...",
    },
    header: {
      dashboard: "Dashboard",
      login: "Log in",
      getStarted: "Get Started",
    },
    appShell: {
      openMenu: "Open menu",
      closeMenu: "Close menu",
      openCommandPalette: "Open command palette",
      searchPlaceholder: "Search or ask...",
      searchFull: "Search procedures, ask AI...",
      collapse: "Collapse",
      expand: "Expand sidebar",
      settings: "Settings",
      logout: "Log out",
      logoAlt: "FormWise Wisp logo",
    },
    askInput: {
      country: "Country:",
      selectCountryHelp: "Select the country for this procedure",
      followUpMode: "Follow-up Mode",
      placeholder: "Ask about bureaucratic procedures in {country}...",
      attach: "Attach Document",
      attachHelp: "PDF or images up to 10MB",
      analyzing: "Analyzing...",
      ask: "Ask",
      askFollowUp: "Ask Follow-up",
      followUpTip:
        "Tip: You can ask about specific steps, documents, costs, or any other detail from the previous answer. The context of your previous question is automatically included.",
      dropHere: "Drop your document here",
      selectCountry: "Select country",
    },
    askPage: {
      title: "Ask FormWise",
      subtitle:
        "Ask any question about bureaucratic procedures and get detailed, source-backed guidance.",
      hideHistory: "Hide History",
      showHistory: "Show History",
      historyTitle: "Conversation History",
      you: "You",
      searchingSources: "Searching official sources...",
      compiling: "FormWise is compiling detailed guidance with legal references",
      suggestionsLabel: "Try asking about:",
      detailPrompt: "Please provide more details",
      moreInfoBody: "For {country}, we need specific details to help you better.",
      needSpecificInfo: "We need more specific information about your question for {country}.",
      missingSpecificInfo:
        "We couldn't find specific information for {country}. Please try a more specific question or check if you're asking about the correct country.",
      failedAnalyze: "Failed to analyze your question. Please try again.",
      failedAnswer: "Failed to get answer. Please try again.",
      checkConnection: "Check your connection and try again.",
      analysisComplete: "Analysis complete",
      analysisDoneFor: "Document analyzed for {country} jurisdiction.",
      answerReady: "Answer ready",
      procedureInfoFor: "Procedure information for {country}.",
      searchingFor: "Searching for {country}...",
      providingGuidance: "FormWise is providing detailed guidance.",
      detailedReady: "Detailed answer ready",
      detailedInfoFor: "Information for {country}: {procedure}",
      selectCountry: "Please select a country",
      selectCountryDescription: "This helps us provide accurate information.",
      selectCountryFirst: "Please select a country first",
      loadingFor: "Analyzing for {country}...",
      loadingDescription: "FormWise is searching the knowledge base and official sources.",
      viewInChat: "View in Chat",
      newQuestion: "New Question",
      followUpTitle: "Follow-up Questions",
      followUpSubtitle: "Ask a follow-up question to get more specific details",
      lastUpdated: "Last updated: {date}",
      suggestions: {
        s1: "How do I get a residence permit?",
        s2: "What documents do I need to register a company?",
        s3: "How to apply for a work visa?",
        s4: "What is the process to renew my passport?",
      },
      followUps: {
        requirements: "What are the exact requirements?",
        timeline: "How long does this take?",
        cost: "What is the total cost?",
        location: "Where exactly do I need to go?",
        visaWork: "Can I work with this visa?",
        visaFamily: "Can I bring my family?",
        visaRejection: "What happens if my application is rejected?",
        permitExtend: "Can I extend this permit?",
        permitRenewal: "What are the conditions for renewal?",
        permitStatus: "Can I change my status?",
        catRequirements: "Requirements",
        catTimeline: "Timeline",
        catCosts: "Costs",
        catLocation: "Location",
        catWorkRights: "Work Rights",
        catFamily: "Family",
        catRejection: "Rejection",
        catExtension: "Extension",
        catRenewal: "Renewal",
        catStatusChange: "Status Change",
      },
      moreInfoTips: {
        intro: "Try including specific details like:",
        one: 'The exact type of procedure (e.g., "work permit" not just "work")',
        two: "Your nationality or current status",
        three: "Specific city or region if relevant",
      },
      responseReceived: "Response received",
      temporaryChat: "Temporary chat",
      temporaryChatShort: "Temporary",
      temporaryChatDescription: "Keep this conversation only in the current browser session. It will not be saved to history, dashboard activity, priorities, or tracked processes.",
      notSaved: "Not saved",
      temporaryEnabled: "Temporary chat enabled",
      temporaryEnabledBody: "Messages in this conversation will stay local to this screen only.",
      loadedFromHistory: "Loaded from history",
      loadedFromHistoryBody: "You can continue this conversation or ask a follow-up.",
      continuingFromHistory: "Continuing from history",
      trackProcess: "Track Process",
      trackedProcess: "Tracked",
      trackingDisabledTemporary: "Temporary chats cannot create tracked processes.",
      processTracked: "Process tracked",
      validation: {
        questionTooShort: "Question is too short. Please provide more details.",
        addMoreContext: "Add more context about your situation",
        includeProcedureType: "Include the type of procedure (e.g., visa, permit, certificate)",
        noCountrySelected: "No country selected. Please select a country.",
        selectCountryForProcedure: "Select the country where you need this procedure",
        vagueTerms: "Question contains vague terms. Please be more specific.",
        replaceVagueWords: "Replace vague words with specific details",
        missingQuestionMark: "Please end your question with a question mark (?).",
        phraseClearly: "Please phrase your question clearly.",
        startWithQuestionWord: "Start with 'How', 'What', 'Where', 'When', or 'Can'",
        needsMoreDetailsTitle: "Question needs more details",
        pleaseBeMoreSpecific: "Please be more specific",
      },
      missingContextTitle: "More context would improve this answer",
      missingContextBody: "The model answered safely, but it flagged details that would make the procedure more accurate. Add them here and FormWise will continue with the full context.",
      missingFields: "Missing fields",
      clarifyingQuestions: "Clarifying questions",
      contextPlaceholder: "Example: I am a non-EU citizen, currently in Sofia, applying for a long-term residence permit before my current visa expires...",
      contextSupplied: "Context supplied",
      sendWithContext: "Send with context",
    },
    confidence: {
      title: "AI confidence",
      body: "This score combines the model's self-check with source quality, structured completeness, and missing-context signals.",
      needsContext: "The answer is useful, but the model says more context is needed before relying on it fully.",
      high: "High confidence",
      good: "Good confidence",
      verify: "Verify details",
      low: "Low confidence",
      unrated: "Unrated",
    },
    answerDisplay: {
      verified: "Verified",
      verifiedLabel: "Verified:",
      legalFoundation: "Legal Foundation",
      law: "Law:",
      article: "Article:",
      year: "Year:",
      source: "Source:",
      officialSource: "Official Source",
      scopeTitle: "Scope of This Procedure",
      scopeCovers: "This Procedure Covers",
      scopeDoesNotCover: "Does NOT Cover",
      eligibility: "Eligibility Requirements",
      whoCanApply: "Who Can Apply",
      prerequisites: "Prerequisites",
      exceptions: "Exceptions",
      whoShouldNotApply: "Who Should NOT Apply",
      timeline: "Processing Timeline",
      minimumTime: "Minimum Time",
      maximumTime: "Maximum Time",
      varies: "Varies",
      expeditedAvailable: "Expedited Option Available",
      time: "Time",
      contactOffice: "Contact office",
      additionalCost: "Additional Cost",
      timelineFactors: "Factors That May Extend Processing",
      afterApproval: "After Approval:",
      costBreakdown: "Cost Breakdown",
      governmentFees: "Government Fees",
      translationCosts: "Translation Costs",
      notarizationCosts: "Notarization Costs",
      otherCosts: "Other Costs",
      totalEstimatedCost: "Total Estimated Cost",
      paymentMethods: "Accepted Payment Methods:",
      stepsToComplete: "Steps to Complete",
      completedCount: "{completed}/{total} completed",
      potentialIssues: "Potential Issues:",
      requiredDocuments: "Required Documents",
      readyCount: "{ready}/{total} ready",
      required: "Required",
      optional: "Optional",
      translationRequired: "Translation Required",
      whereToGet: "Where to get:",
      validFor: "Valid for:",
      cost: "Cost:",
      legalBasis: "Legal basis:",
      requirements: "Requirements:",
      detailedInstructions: "Detailed Instructions",
      steps: "Steps",
      fullResponse: "Full Response",
      noDetailedInstructions: "No detailed instructions available.",
      whereToGo: "Where to Go",
      appointmentRequired: "Appointment required",
      visitWebsite: "Visit Website",
      viewOnMap: "View on Map",
      importantWarnings: "Important Warnings",
      commonRejections: "Common Reasons for Rejection",
      howToAvoid: "How to avoid:",
      scamsToAvoid: "Scams to Avoid",
      whatNotToDo: "What NOT to Do",
      additionalNotes: "Additional Notes",
      relatedProcedures: "Related Procedures",
      doFirst: "Do First",
      doNext: "Do Next",
      alternative: "Alternative",
    },
    processes: {
      title: "Ongoing Processes",
      empty: "No ongoing processes",
      startInquiry: "Start a new inquiry",
      processingAnswer: "Waiting for FormWise answer",
      questionSubmitted: "Question submitted",
      questionSubmittedBody: "FormWise received the request and started processing it.",
      answerGeneration: "Answer generation",
      answerGenerationBody: "Searching sources and preparing the response.",
      autoCreatedNotes: "Auto-created from the latest question.",
      retryQuestion: "Retry the question",
      processingFailedNotes: "The request could not be completed for:",
      next: "Next:",
      progress: "Progress",
      percentComplete: "{percent}% complete",
      overallProgress: "Overall Progress",
      nextDeadline: "Next Deadline",
      estimatedCompletion: "Est. Completion",
      location: "Location",
      notes: "Notes",
      notesPlaceholder: "Add private notes about this process...",
      steps: "Process Steps",
      current: "Current",
      upcoming: "Upcoming",
      documents: "Required Documents",
      newDocumentPlaceholder: "Add a document or file checklist item...",
      addDocument: "Add document",
      askQuestion: "Ask a Question",
    },
    theme: {
      change: "Change theme",
      colorTheme: "Color Theme",
      appearance: "Appearance",
      warm: "Warm",
      fresh: "Fresh",
      light: "Light",
      dark: "Dark",
      system: "System",
    },
    commandPalette: {
      title: "FormWise Command Palette",
      description: "Search procedures, jump pages, switch country, ask AI.",
      placeholder: "Type a command, search a procedure, or ask AI...",
      navigation: "Navigation",
      quickAsk: "Quick Ask",
      procedureCategories: "Procedure Categories",
      procedureCount: "{count} procedures",
      switchCountry: "Switch Country",
      appearance: "Appearance",
      account: "Account",
      switchToLight: "Switch to Light",
      switchToDark: "Switch to Dark",
      footer: "FormWise · Bureaucracy AI",
      navigate: "Navigate",
      open: "Open",
      quickAskResidence: "How do I get a residence permit?",
      quickAskCompany: "What documents do I need to register a company?",
      quickAskPassport: "How to renew my passport?",
      quickAskLicense: "What's the process for getting a driver's license?",
    },
    auth: {
      login: {
        title: "Welcome back",
        subtitle: "Log in to your account to continue",
        forgot: "Forgot password?",
        passwordPlaceholder: "Enter your password",
        loggingIn: "Logging in...",
        orContinue: "Or continue with",
        socialSoon: "Social login coming soon",
        noAccount: "Don't have an account?",
        failed: "Login failed",
      },
      register: {
        title: "Create your account",
        subtitle: "Start navigating bureaucracy with AI",
        namePlaceholder: "John Doe",
        passwordPlaceholder: "At least 6 characters",
        creating: "Creating account...",
        benefitsTitle: "What you get:",
        already: "Already have an account?",
        failed: "Registration failed",
        passwordTooShort: "Password must be at least 6 characters",
        features: {
          one: "Unlimited document analysis",
          two: "Process tracking dashboard",
          three: "Document history",
          four: "Priority support",
        },
      },
    },
    browse: {
      title: "Browse Procedures",
      subtitle: "Explore all available bureaucratic procedures by category.",
      askInstead: "Ask AI Instead",
      bannerTitle: "More Countries Coming Soon",
      bannerBody:
        "We're expanding to cover more countries. Currently featuring procedures for Bulgaria.",
      prompt: "Can't find what you need?",
      askAi: "Ask AI",
      loading: "Loading...",
    },
    history: {
      title: "History",
      subtitle: "Browse your past questions and document analyses.",
      searchPlaceholder: "Search history...",
      noResultsBody: "Try adjusting your search or filter criteria.",
      askNew: "Ask a new question",
      clearAll: "Clear All",
      noHistory: "No history yet",
      startAsking: "Start asking questions to build your history",
      askFirst: "Ask your first question",
      detailTitle: "History detail",
      prompt: "Prompt",
      response: "Response",
      confidence: "Confidence",
      steps: "Key steps",
      step: "Step",
      documents: "Documents",
      rawResponse: "Raw response",
      noResponseSaved: "No full response was saved for this item.",
      continueInAsk: "Continue in Ask",
      delete: "Delete",
    },
    dashboard: {
      badge: "FormWise · AI assistant for European bureaucracy",
      morning: "Good morning",
      afternoon: "Good afternoon",
      evening: "Good evening",
      friend: "friend",
      summary:
        "Here's a snapshot of your bureaucratic journey. Press {shortcut} any time to search, navigate, or ask AI.",
      newQuestion: "New Question",
      openPalette: "or open command palette →",
      welcomeTitle: "Welcome to FormWise!",
      welcomeBody:
        "Ask our AI assistant about any bureaucratic process and get step-by-step guidance.",
      quickActions: "Quick Actions",
      quickAskTitle: "Ask a Question",
      quickAskBody: "Get instant answers about any procedure",
      quickUploadTitle: "Upload Document",
      quickUploadBody: "Analyze documents for guidance",
      quickBrowseTitle: "Browse Guides",
      quickBrowseBody: "Explore procedure categories",
      quickHelpTitle: "Get Help",
      quickHelpBody: "Tips and support resources",
      welcomeAction: "Get Started",
      todayPriorities: "Today's priorities",
      planWithAi: "Plan with AI",
      viewTodayHistory: "View today's chats",
      noPriorities: "No priorities set",
      noChatsToday: "No chats saved today",
      getStartedWithAi: "Get started with AI",
      recentActivity: "Recent Activity",
      noRecentActivity: "No recent activity",
      noRecentActivityBody: "Ask a question to see your activity here",
      activityThisWeek: "Activity this week",
      activityThisWeekBody: "Completed answers vs questions asked",
      statsDocuments: "Documents Analyzed",
      statsDocumentsBody: "Total document analyses",
      statsQuestions: "Questions Asked",
      statsQuestionsBody: "Total questions",
      statsCompletedBody: "Successfully answered",
      statsTimeSaved: "Time Saved",
      statsTimeSavedBody: "Hours estimated",
      priorityCritical: "Critical",
      prioritySoon: "Soon",
      priorityUpcoming: "Upcoming",
      due: "due",
      left: "left",
    },
    clippy: {
      assistantLabel: "FormWise assistant",
      nextTip: "Next tip",
      hide: "Hide for this session",
      restore: "Restore FormWise helper",
      dragHint: "Drag me anywhere",
      resetPosition: "Reset position",
      tips: {
        default: {
          one: "Need a hand with the bureaucracy maze? I can point you to the right flow.",
          two: "Upload documents when the exact wording matters. Forms love tiny details.",
          three: "If an answer feels too broad, ask a follow-up with your status, city, and deadline.",
        },
        dashboard: {
          one: "Your dashboard now only reflects your own saved questions and tracked processes.",
          two: "Use Track Process after an answer to turn guidance into an ongoing checklist.",
          three: "Empty states are good here: they mean no fake paperwork goblins are haunting your data.",
        },
        ask: {
          one: "Temporary chat keeps experiments off your history and dashboard.",
          two: "When FormWise asks for missing context, answer the panel and it will continue with the full thread.",
          three: "Strong prompts include country, status, deadline, and the exact procedure name.",
        },
        history: {
          one: "Open any history item to inspect the saved prompt, answer, confidence, and next steps.",
          two: "Continue in Ask restores the saved answer as conversation context.",
          three: "History is local to this browser for now, so clearing browser storage clears it too.",
        },
      },
    },
    chatNoSpecificInfo:
      "I don't have specific information about this procedure yet. Please check the official government website for {country}.",
    compareInsufficientContext:
      "I do not have enough official source context to compare these countries reliably yet. Please narrow the question or verify on official government websites.",
    journeyLimitedInfo:
      "Limited official procedure information is currently available for {country}.",
    journeyMissingCoverage: "Missing grounded coverage for: {areas}.",
    journeyVerifyRequirements:
      "Verify visa, registration, and permit requirements on official government websites before acting.",
    compareExcludedCountry:
      "{code} was excluded because official source coverage was too weak for a reliable comparison.",
    compareMissingStructuredCountry:
      "{code} had source coverage but did not produce a sufficiently grounded structured comparison entry.",
    compareNoGroundedOutput:
      "I do not have enough grounded country-specific output to compare these options reliably.",
    journeyMissingPhases:
      "Some expected roadmap phases were missing from the model output and were reinserted as empty placeholders: {phases}.",
    journeyLimitedCoverage:
      "Some parts of this plan rely on limited official source coverage. Double-check these areas on official sources: {areas}.",
    journeyNoStrongGrounding:
      "No journey areas were strongly grounded in official source context.",
    settings: {
      title: "Settings",
      subtitle: "Configure your AI provider, account, notifications, and privacy preferences.",
      tabs: {
        ai: "AI Models",
        billing: "Billing",
        account: "Account",
        notifications: "Notifications",
        privacy: "Privacy",
        developer: "Developer",
      },
      common: {
        save: "Save",
        saving: "Saving...",
        reset: "Reset to defaults",
        loading: "Loading...",
      },
      ai: {
        defaultProvider: "Default AI Provider",
        defaultProviderDesc: "Choose which provider answers your questions. The environment default is",
        envDefaultSuffix: ". Selection here overrides it for your account.",
        provider: "Provider",
        model: "Model",
        notConfigured: "(not configured)",
        notConfiguredTitle: "Provider not configured",
        notConfiguredHint: "Set the relevant environment variable in",
        params: "Generation parameters",
        paramsDesc: "Fine-tune sampling. Defaults to provider-controlled values when blank.",
        temperature: "Temperature",
        temperatureHint: "Lower = deterministic. Higher = creative. Recommended: 0.2–0.7 for legal accuracy.",
        maxTokens: "Max tokens",
        maxTokensPlaceholder: "Provider default (usually 4096)",
        systemPromptOverride: "System prompt override",
        systemPromptPlaceholder: "Leave empty to use the built-in FormWise system prompt.",
        systemPromptHint: "Advanced: replace the default bureaucracy expert prompt. Leave empty to keep the built-in one.",
        fallbackChain: "Fallback chain",
        fallbackChainDesc: "If the primary provider fails, the router walks this list in order. Configured providers appear first.",
        providerStatus: "Provider status",
        providerStatusDesc: "Live configuration check. Click \"Test\" to verify the connection with a tiny ping prompt.",
        defaultLabel: "Default",
        testButton: "Test",
        testFailed: "failed",
        savedToast: "Preferences saved",
        resetToast: "Reverted to environment defaults",
      },
      billing: {
        currentPlan: "Current plan",
        period: "Period",
        renews: "Renews",
        manage: "Manage subscription",
        questions: "Questions (/api/ask)",
        documentAnalyses: "Document analyses",
        relocationPlans: "Relocation plans",
        countryComparisons: "Country comparisons",
        unlimited: "unlimited",
        recommended: "Recommended",
        currentPlanLabel: "Current plan",
        cancelViaPortal: "Cancel via portal",
        free: "Free",
        upgradeToPro: "Upgrade to Pro",
        upgradeToBusiness: "Upgrade to Business",
        perMonth: "/ month",
        billingFooter: "Billing handled by LemonSqueezy — VAT and EU compliance included. You can cancel anytime through \"Manage subscription\".",
      },
      account: {
        profile: "Profile",
        profileDesc: "Account info shown on your dashboard.",
        displayName: "Display name",
        email: "Email",
        savePhase2: "Save (Phase 2)",
        password: "Password",
        passwordDesc: "Phase 1 stores credentials in localStorage. Real password change requires the backend auth from Phase 2.",
        signOut: "Sign out",
        signOutDesc: "End the session on this device.",
        signOutButton: "Sign out",
        deleteTitle: "Delete account",
        deleteDesc: "Permanently remove your account and all locally stored history. Cannot be undone.",
        deleteButton: "Delete my account",
        deleteConfirm: "Yes, delete forever",
        cancel: "Cancel",
      },
      notifications: {
        title: "Notification preferences",
        desc: "Control how and when FormWise reaches out. Email-based notifications activate with the Resend integration in Phase 3.",
        inAppToasts: "In-app toasts",
        inAppToastsDesc: "Status updates and confirmations as you use FormWise.",
        emailReminders: "Step reminders",
        emailRemindersDesc: "Email reminders before procedure steps come due.",
        emailDeadlines: "Deadline alerts",
        emailDeadlinesDesc: "Email warnings 7 / 3 / 1 day before legal deadlines.",
        govSourceUpdates: "Government source updates",
        govSourceUpdatesDesc: "Notify when an official page changes for a procedure you've tracked.",
        weeklyDigest: "Weekly digest",
        weeklyDigestDesc: "Summary of your active procedures every Monday.",
        productAnnouncements: "Product announcements",
        productAnnouncementsDesc: "New features, model upgrades, and important notices.",
      },
      privacy: {
        exportTitle: "Data export",
        exportDesc: "Download everything FormWise stores about you on this device. The export is a JSON file you can keep or import elsewhere later.",
        exportButton: "Export my data",
        exportSuccess: "Data exported",
        conversationTitle: "Conversation privacy",
        conversationDesc: "Control whether FormWise saves your conversation history.",
        defaultTemporary: "Default to Temporary Chat",
        defaultTemporaryDesc: "When enabled, new conversations are not saved to history by default. You can still toggle per-conversation on the Ask page.",
        redactPii: "Redact PII before sending to model",
        redactPiiDesc: "Auto-mask EGN, phone, IBAN, and email in your messages before they leave your browser. Reduces data exposure to the AI provider.",
        telemetryTitle: "Telemetry",
        telemetryDesc: "Anonymous usage analytics to help us improve. No prompts or responses are sent — only event names and timing.",
        telemetryToggle: "Allow product analytics",
        telemetryToggleDesc: "Sends anonymized events (e.g. \"asked question\", \"saved process\"). Never includes prompt text or personal info.",
        clearTitle: "Clear conversation history",
        clearDesc: "Removes all saved questions, processes, and activity from this device. Cannot be undone.",
        clearButton: "Clear all history",
        clearConfirm: "Yes, clear everything",
        clearedToast: "History cleared on this device",
      },
    },
  },
  bg: {
    language: {
      label: "Език",
      change: "Смени езика",
      english: "Английски",
      bulgarian: "Български",
      german: "Немски",
    },
    nav: {
      dashboard: "Табло",
      ask: "Попитай",
      browse: "Разгледай",
      history: "История",
    },
    common: {
      loading: "Зареждане...",
      close: "Затвори",
      clear: "Изчисти",
      search: "Търси",
      email: "Имейл",
      password: "Парола",
      fullName: "Име и фамилия",
      noResults: "Няма резултати",
      tryAgain: "Опитай отново",
      getStarted: "Започни",
      createAccount: "Създай акаунт",
      logIn: "Вход",
      signUp: "Регистрация",
      viewAll: "Виж всички",
      all: "Всички",
      questions: "Въпроси",
      documents: "Документи",
      completed: "Завършено",
      inProgress: "В процес",
      waiting: "Изчакване",
      submitted: "Подаден",
      pending: "Чакащ",
      missing: "Липсващ",
    },
    landing: {
      trustBadges: {
        secure: "Сигурно и поверително",
        instant: "Моментален анализ",
        users: "10k+ потребители",
      },
      hero: {
        badge: "AI насоки за всеки процес",
        titlePrefix: "Премини през бюрокрацията",
        titleHighlight: "без главоболия",
        subtitle:
          "Качи документи, задай въпроси и получи насоки стъпка по стъпка за всеки административен процес. От визи до бизнес разрешителни.",
        tryFree: "Пробвай безплатно",
        seeHow: "Виж как работи",
        trust: "Не е нужна карта. 1 безплатен анализ за старт.",
        docLabel: "Документ",
        docValue: "Анализиран",
        stepLabel: "Стъпка 3 от 5",
        stepValue: "Завършена",
      },
      demo: {
        title: "Пробвай сега",
        subtitle:
          "Задай въпрос за бюрократични процедури и виж как FormWise може да помогне.",
        placeholder: "Попитай за бюрократична процедура...",
        tryLabel: "Опитай:",
        ctaText:
          "Искаш пълен достъп? Създай безплатен акаунт, за да следиш процесите си.",
        ctaButton: "Създай акаунт",
        samples: {
          residence: "Как да кандидатствам за разрешение за пребиваване в България?",
          visa: "Какви документи ми трябват за работна виза?",
          company: "Как да регистрирам фирма в чужбина?",
        },
      },
      features: {
        title: "Всичко нужно, за да овладееш документите",
        subtitle:
          "Мощни функции, създадени да направят бюрократичните процеси по-прости и по-малко стресиращи.",
        items: {
          analysisTitle: "Анализ на документи",
          analysisDescription:
            "Качи документ и получи моментални изводи. AI разпознава формуляри, заявления и официални писма.",
          guidanceTitle: "Насоки стъпка по стъпка",
          guidanceDescription:
            "Получаваш ясни, номерирани стъпки за всяка процедура. Знаеш точно какво следва.",
          supportTitle: "Поддръжка за много държави",
          supportDescription:
            "Независимо дали си у дома или в чужбина, получаваш локализирани насоки за различни държави.",
          timeTitle: "Оценка на времето",
          timeDescription:
            "Виж колко време отнема всяка стъпка. Планирай по-добре и избягвай излишни забавяния.",
          checklistTitle: "Списък с документи",
          checklistDescription:
            "Интерактивни списъци ти помагат да подготвиш всичко необходимо и да следиш напредъка си.",
          askTitle: "Питай всичко",
          askDescription:
            "Имаш конкретен въпрос? Просто попитай. Получаваш бързи и точни отговори.",
        },
      },
      authPrompt: {
        title: "Създай безплатен акаунт",
        body:
          "Използва безплатния си опит. Създай акаунт, за да продължиш с FormWise и да следиш бюрократичните си процеси.",
        later: "По-късно",
        create: "Създай акаунт",
        existing: "Вече имаш акаунт? Влез",
      },
      footer: {
        product: "FormWise - AI навигация през бюрокрацията",
        tagline: "Създаден за хората в сложни административни процеси",
      },
      loading: "Анализираме въпроса ти...",
    },
    header: {
      dashboard: "Табло",
      login: "Вход",
      getStarted: "Започни",
    },
    appShell: {
      openMenu: "Отвори меню",
      closeMenu: "Затвори меню",
      openCommandPalette: "Отвори командната палитра",
      searchPlaceholder: "Търси или попитай...",
      searchFull: "Търси процедури, попитай AI...",
      collapse: "Свий",
      expand: "Разгъни страничното меню",
      settings: "Настройки",
      logout: "Изход",
      logoAlt: "Лого Wisp на FormWise",
    },
    askInput: {
      country: "Държава:",
      selectCountryHelp: "Избери държавата за тази процедура",
      followUpMode: "Режим за уточнение",
      placeholder: "Попитай за бюрократични процедури в {country}...",
      attach: "Прикачи документ",
      attachHelp: "PDF или изображения до 10MB",
      analyzing: "Анализиране...",
      ask: "Попитай",
      askFollowUp: "Попитай допълнително",
      followUpTip:
        "Съвет: можеш да питаш за конкретни стъпки, документи, разходи или друг детайл от предишния отговор. Контекстът се включва автоматично.",
      dropHere: "Пусни документа тук",
      selectCountry: "Избери държава",
    },
    askPage: {
      title: "Попитай FormWise",
      subtitle:
        "Задай въпрос за бюрократични процедури и получи подробни насоки с източници.",
      hideHistory: "Скрий историята",
      showHistory: "Покажи историята",
      historyTitle: "История на разговора",
      you: "Ти",
      searchingSources: "Търсене в официални източници...",
      compiling: "FormWise събира подробни насоки с правни препратки",
      suggestionsLabel: "Опитай да попиташ:",
      detailPrompt: "Моля, добави повече детайли",
      moreInfoBody: "За {country} ни трябват по-конкретни детайли, за да помогнем.",
      needSpecificInfo:
        "Нужна ни е по-конкретна информация за въпроса ти относно {country}.",
      missingSpecificInfo:
        "Не открихме достатъчно конкретна информация за {country}. Опитай с по-специфичен въпрос или провери дали държавата е правилна.",
      failedAnalyze: "Неуспешен анализ на въпроса. Опитай отново.",
      failedAnswer: "Не успяхме да получим отговор. Опитай отново.",
      checkConnection: "Провери връзката си и опитай пак.",
      analysisComplete: "Анализът е готов",
      analysisDoneFor: "Документът е анализиран за юрисдикцията на {country}.",
      answerReady: "Отговорът е готов",
      procedureInfoFor: "Информация за процедурата в {country}.",
      searchingFor: "Търсене за {country}...",
      providingGuidance: "FormWise подготвя подробни насоки.",
      detailedReady: "Подробният отговор е готов",
      detailedInfoFor: "Информация за {country}: {procedure}",
      selectCountry: "Моля, избери държава",
      selectCountryDescription: "Това ни помага да дадем точна информация.",
      selectCountryFirst: "Първо избери държава",
      loadingFor: "Анализ за {country}...",
      loadingDescription: "FormWise търси в базата знания и официални източници.",
      viewInChat: "Виж в чата",
      newQuestion: "Нов въпрос",
      followUpTitle: "Допълнителни въпроси",
      followUpSubtitle: "Задай уточняващ въпрос за по-конкретни детайли",
      lastUpdated: "Последна актуализация: {date}",
      suggestions: {
        s1: "Как да получа разрешение за пребиваване?",
        s2: "Какви документи трябват за регистрация на фирма?",
        s3: "Как се кандидатства за работна виза?",
        s4: "Каква е процедурата за подновяване на паспорт?",
      },
      followUps: {
        requirements: "Какви са точните изисквания?",
        timeline: "Колко време отнема?",
        cost: "Каква е общата цена?",
        location: "Къде точно трябва да отида?",
        visaWork: "Мога ли да работя с тази виза?",
        visaFamily: "Мога ли да доведа семейството си?",
        visaRejection: "Какво става, ако молбата бъде отказана?",
        permitExtend: "Мога ли да удължа това разрешение?",
        permitRenewal: "Какви са условията за подновяване?",
        permitStatus: "Мога ли да сменя статуса си?",
        catRequirements: "Изисквания",
        catTimeline: "Срок",
        catCosts: "Разходи",
        catLocation: "Място",
        catWorkRights: "Право на работа",
        catFamily: "Семейство",
        catRejection: "Отказ",
        catExtension: "Удължаване",
        catRenewal: "Подновяване",
        catStatusChange: "Смяна на статут",
      },
      moreInfoTips: {
        intro: "Опитай да добавиш детайли като:",
        one: 'Точния вид процедура (напр. "разрешение за работа", а не само "работа")',
        two: "Твоята националност или текущ статут",
        three: "Конкретен град или регион, ако е важно",
      },
      responseReceived: "Получен отговор",
      trackProcess: "Проследи процеса",
      trackedProcess: "Проследено",
      trackingDisabledTemporary: "Временните чатове не могат да създават проследявани процеси.",
      processTracked: "Процесът е проследен",
      validation: {
        questionTooShort: "Въпросът е твърде кратък. Моля, добави повече детайли.",
        addMoreContext: "Добави повече контекст за ситуацията си",
        includeProcedureType: "Включи вида на процедурата (напр. виза, разрешение, сертификат)",
        noCountrySelected: "Не е избрана държава. Моля, избери държава.",
        selectCountryForProcedure: "Избери държавата, за която ти трябва тази процедура",
        vagueTerms: "Въпросът съдържа неясни думи. Моля, бъди по-конкретен.",
        replaceVagueWords: "Замени неясните думи с конкретни детайли",
        missingQuestionMark: "Моля, завърши въпроса с въпросителен знак (?).",
        phraseClearly: "Моля, формулирай въпроса ясно.",
        startWithQuestionWord: "Започни с „Как“, „Какво“, „Къде“, „Кога“ или „Мога ли“",
        needsMoreDetailsTitle: "Въпросът се нуждае от повече детайли",
        pleaseBeMoreSpecific: "Моля, бъди по-конкретен",
      },
      temporaryChatShort: "Временно",
      continuingFromHistory: "Продължение от историята",
    },
    answerDisplay: {
      verified: "Проверено",
      verifiedLabel: "Проверено:",
      legalFoundation: "Правно основание",
      law: "Закон:",
      article: "Член:",
      year: "Година:",
      source: "Източник:",
      officialSource: "Официален източник",
      scopeTitle: "Обхват на процедурата",
      scopeCovers: "Процедурата покрива",
      scopeDoesNotCover: "Не покрива",
      eligibility: "Условия за допустимост",
      whoCanApply: "Кой може да кандидатства",
      prerequisites: "Предварителни условия",
      exceptions: "Изключения",
      whoShouldNotApply: "Кой не трябва да кандидатства",
      timeline: "Срок за обработка",
      minimumTime: "Минимално време",
      maximumTime: "Максимално време",
      varies: "Варира",
      expeditedAvailable: "Има ускорена опция",
      time: "Време",
      contactOffice: "Свържи се с институцията",
      additionalCost: "Допълнителна цена",
      timelineFactors: "Фактори, които могат да удължат срока",
      afterApproval: "След одобрение:",
      costBreakdown: "Разбивка на разходите",
      governmentFees: "Държавни такси",
      translationCosts: "Разходи за превод",
      notarizationCosts: "Нотариални разходи",
      otherCosts: "Други разходи",
      totalEstimatedCost: "Обща ориентировъчна цена",
      paymentMethods: "Приети методи на плащане:",
      stepsToComplete: "Стъпки за изпълнение",
      completedCount: "{completed}/{total} завършени",
      potentialIssues: "Възможни проблеми:",
      requiredDocuments: "Необходими документи",
      readyCount: "{ready}/{total} готови",
      required: "Задължителен",
      optional: "Незадължителен",
      translationRequired: "Изисква се превод",
      whereToGet: "Откъде се получава:",
      validFor: "Валиден за:",
      cost: "Цена:",
      legalBasis: "Правно основание:",
      requirements: "Изисквания:",
      detailedInstructions: "Подробни инструкции",
      steps: "Стъпки",
      fullResponse: "Пълен отговор",
      noDetailedInstructions: "Няма подробни инструкции.",
      whereToGo: "Къде да отидеш",
      appointmentRequired: "Необходим е час",
      visitWebsite: "Посети сайта",
      viewOnMap: "Виж на картата",
      importantWarnings: "Важни предупреждения",
      commonRejections: "Чести причини за отказ",
      howToAvoid: "Как да избегнеш:",
      scamsToAvoid: "Измами, които да избягваш",
      whatNotToDo: "Какво да НЕ правиш",
      additionalNotes: "Допълнителни бележки",
      relatedProcedures: "Свързани процедури",
      doFirst: "Първо направи",
      doNext: "След това",
      alternative: "Алтернатива",
    },
    processes: {
      title: "Текущи процеси",
      empty: "Няма текущи процеси",
      startInquiry: "Започни ново запитване",
      next: "Следващо:",
      progress: "Напредък",
      percentComplete: "{percent}% завършено",
      overallProgress: "Общ напредък",
      nextDeadline: "Следващ срок",
      estimatedCompletion: "Очаквано завършване",
      location: "Място",
      notes: "Бележки",
      notesPlaceholder: "Добави лични бележки за процеса...",
      steps: "Стъпки на процеса",
      current: "Текуща",
      upcoming: "Предстояща",
      documents: "Необходими документи",
      newDocumentPlaceholder: "Добави документ или елемент към списъка...",
      addDocument: "Добави документ",
      askQuestion: "Задай въпрос",
    },
    auth: {
      login: {
        title: "Добре дошъл отново",
        subtitle: "Влез в акаунта си, за да продължиш",
        forgot: "Забравена парола?",
        passwordPlaceholder: "Въведи паролата си",
        loggingIn: "Влизане...",
        orContinue: "Или продължи с",
        socialSoon: "Социалният вход идва скоро",
        noAccount: "Нямаш акаунт?",
        failed: "Неуспешен вход",
      },
      register: {
        title: "Създай своя акаунт",
        subtitle: "Започни да навигираш бюрокрацията с AI",
        namePlaceholder: "Иван Иванов",
        passwordPlaceholder: "Поне 6 символа",
        creating: "Създаване на акаунт...",
        benefitsTitle: "Какво получаваш:",
        already: "Вече имаш акаунт?",
        failed: "Неуспешна регистрация",
        passwordTooShort: "Паролата трябва да е поне 6 символа",
        features: {
          one: "Неограничен анализ на документи",
          two: "Табло за проследяване на процеси",
          three: "История на документите",
          four: "Приоритетна поддръжка",
        },
      },
    },
    browse: {
      title: "Разгледай процедури",
      subtitle: "Разгледай наличните бюрократични процедури по категории.",
      askInstead: "Попитай AI вместо това",
      bannerTitle: "Още държави идват скоро",
      bannerBody:
        "Разширяваме обхвата с още държави. В момента акцентът е върху процедури за България.",
      prompt: "Не намираш каквото ти трябва?",
      askAi: "Попитай AI",
      loading: "Зареждане...",
    },
    history: {
      title: "История",
      subtitle: "Разгледай предишните си въпроси и анализи на документи.",
      searchPlaceholder: "Търси в историята...",
      noResultsBody: "Опитай да промениш търсенето или филтрите.",
      askNew: "Задай нов въпрос",
    },
    dashboard: {
      badge: "FormWise · AI асистент за европейска бюрокрация",
      morning: "Добро утро",
      afternoon: "Добър ден",
      evening: "Добър вечер",
      friend: "приятелю",
      summary:
        "Ето кратък преглед на бюрократичния ти път. Натисни {shortcut} по всяко време, за да търсиш, навигираш или попиташ AI.",
      newQuestion: "Нов въпрос",
      openPalette: "или отвори командната палитра →",
      welcomeTitle: "Добре дошъл във FormWise!",
      welcomeBody:
        "Попитай AI асистента ни за всяка бюрократична процедура и получи насоки стъпка по стъпка.",
      quickActions: "Бързи действия",
      welcomeAction: "Започни",
      todayPriorities: "Днешни приоритети",
      planWithAi: "Планирай с AI",
      noPriorities: "Няма зададени приоритети",
      getStartedWithAi: "Започни с AI",
      priorityCritical: "Критично",
      prioritySoon: "Скоро",
      priorityUpcoming: "Предстоящо",
      due: "срок",
      left: "остават",
    },
    clippy: {
      assistantLabel: "Помощникът на FormWise",
      nextTip: "Следващ съвет",
      hide: "Скрий за тази сесия",
      restore: "Възстанови помощника на FormWise",
      dragHint: "Премести ме където искаш",
      resetPosition: "Нулирай позицията",
      tips: {
        default: {
          one: "Трябва ти помощ с бюрократичния лабиринт? Ще те насоча към правилния поток.",
          two: "Качвай документи, когато точната формулировка има значение. Формулярите обичат дребните детайли.",
          three: "Ако отговорът е твърде общ, задай уточняващ въпрос със статус, град и краен срок.",
        },
        dashboard: {
          one: "Таблото ти вече показва само твоите запазени въпроси и проследени процеси.",
          two: "Използвай „Проследи процеса“ след отговор, за да превърнеш насоките в активен чеклист.",
          three: "Празните състояния тук са добър знак: означават, че няма измислени хартиени чудовища в данните ти.",
        },
        ask: {
          one: "Временният чат държи експериментите извън историята и таблото.",
          two: "Когато FormWise поиска липсващ контекст, отговори в панела и той ще продължи с цялата нишка.",
          three: "Силните запитвания включват държава, статус, краен срок и точното име на процедурата.",
        },
        history: {
          one: "Отвори който и да е запис в историята, за да прегледаш запазения prompt, отговора, увереността и следващите стъпки.",
          two: "„Continue in Ask“ възстановява запазения отговор като контекст на разговора.",
          three: "Засега историята е локална само за този браузър, така че изчистването на browser storage я изтрива.",
        },
      },
    },
    chatNoSpecificInfo:
      "Все още нямам конкретна информация за тази процедура. Моля, провери официалния правителствен сайт за {country}.",
    compareInsufficientContext:
      "Все още нямам достатъчно официални източници, за да сравня надеждно тези държави. Стесни въпроса или провери официалните правителствени сайтове.",
    journeyLimitedInfo:
      "В момента има ограничена официална информация за процедури в {country}.",
    journeyMissingCoverage: "Липсва надеждно покритие за: {areas}.",
    journeyVerifyRequirements:
      "Провери изискванията за виза, регистрация и разрешителни в официалните правителствени сайтове, преди да действаш.",
    compareExcludedCountry:
      "{code} е изключена, защото покритието от официални източници е твърде слабо за надеждно сравнение.",
    compareMissingStructuredCountry:
      "{code} имаше източници, но не произведе достатъчно надежден структуриран запис за сравнение.",
    compareNoGroundedOutput:
      "Нямам достатъчно надежден изход по държави, за да сравня тези опции.",
    journeyMissingPhases:
      "Някои очаквани фази липсваха в изхода на модела и бяха върнати като празни секции: {phases}.",
    journeyLimitedCoverage:
      "Части от този план разчитат на ограничено покритие от официални източници. Провери следните области: {areas}.",
    journeyNoStrongGrounding:
      "Нито една част от този план не беше силно подкрепена от официален контекст.",
    settings: {
      title: "Настройки",
      subtitle: "Конфигурирай AI доставчика, акаунта, известията и поверителността.",
      tabs: {
        ai: "AI модели",
        billing: "Абонамент",
        account: "Акаунт",
        notifications: "Известия",
        privacy: "Поверителност",
        developer: "Разработчик",
      },
      common: {
        save: "Запази",
        saving: "Запазване...",
        reset: "Върни по подразбиране",
        loading: "Зарежда...",
      },
      ai: {
        defaultProvider: "Доставчик по подразбиране",
        defaultProviderDesc: "Избери кой доставчик да отговаря на въпросите ти. По подразбиране в средата е",
        envDefaultSuffix: ". Изборът тук замества това за твоя акаунт.",
        provider: "Доставчик",
        model: "Модел",
        notConfigured: "(не е конфигуриран)",
        notConfiguredTitle: "Доставчикът не е конфигуриран",
        notConfiguredHint: "Задай съответната env променлива в",
        params: "Параметри на генериране",
        paramsDesc: "Фини настройки за sampling. По подразбиране се ползват стойностите на доставчика.",
        temperature: "Температура",
        temperatureHint: "По-ниско = детерминирано. По-високо = креативно. Препоръчително: 0.2–0.7 за правни отговори.",
        maxTokens: "Макс. токени",
        maxTokensPlaceholder: "По подразбиране (обикновено 4096)",
        systemPromptOverride: "Замяна на system prompt",
        systemPromptPlaceholder: "Остави празно за вградения FormWise prompt.",
        systemPromptHint: "За напреднали: замества основния prompt. Празно = вграденият.",
        fallbackChain: "Fallback верига",
        fallbackChainDesc: "Ако основният доставчик откаже, рутерът обикаля този списък по ред. Конфигурираните са първи.",
        providerStatus: "Статус на доставчиците",
        providerStatusDesc: "Жива проверка на конфигурацията. Натисни „Тест“ за бърз ping prompt.",
        defaultLabel: "По подразбиране",
        testButton: "Тест",
        testFailed: "грешка",
        savedToast: "Настройките са запазени",
        resetToast: "Върнато към стойностите от средата",
      },
      billing: {
        currentPlan: "Текущ план",
        period: "Период",
        renews: "Подновява се",
        manage: "Управление на абонамента",
        questions: "Въпроси (/api/ask)",
        documentAnalyses: "Анализи на документи",
        relocationPlans: "Планове за релокация",
        countryComparisons: "Сравнения между държави",
        unlimited: "неограничено",
        recommended: "Препоръчано",
        currentPlanLabel: "Текущ план",
        cancelViaPortal: "Откажи през портала",
        free: "Безплатно",
        upgradeToPro: "Премини към Pro",
        upgradeToBusiness: "Премини към Business",
        perMonth: "/ месец",
        billingFooter: "Плащанията се обработват от LemonSqueezy — ДДС и EU compliance включени. Можеш да откажеш по всяко време през „Управление на абонамента“.",
      },
      account: {
        profile: "Профил",
        profileDesc: "Информация за акаунта, видима на таблото.",
        displayName: "Име за показване",
        email: "Имейл",
        savePhase2: "Запази (Phase 2)",
        password: "Парола",
        passwordDesc: "Phase 1 пази данните в localStorage. Истинска смяна на парола изисква backend от Phase 2.",
        signOut: "Изход",
        signOutDesc: "Прекрати сесията на това устройство.",
        signOutButton: "Изход",
        deleteTitle: "Изтрий акаунта",
        deleteDesc: "Изтрива акаунта и цялата история необратимо.",
        deleteButton: "Изтрий моя акаунт",
        deleteConfirm: "Да, изтрий завинаги",
        cancel: "Отказ",
      },
      notifications: {
        title: "Настройки на известия",
        desc: "Контролирай как и кога FormWise те търси. Email известията тръгват с Resend интеграцията от Phase 3.",
        inAppToasts: "Известия в приложението",
        inAppToastsDesc: "Статус и потвърждения докато ползваш FormWise.",
        emailReminders: "Напомняния за стъпки",
        emailRemindersDesc: "Email напомняния преди стъпките на процедурата.",
        emailDeadlines: "Сигнал за крайни срокове",
        emailDeadlinesDesc: "Email предупреждения 7 / 3 / 1 ден преди законен срок.",
        govSourceUpdates: "Промени в правителствени източници",
        govSourceUpdatesDesc: "Уведомление при промяна на официална страница на следена процедура.",
        weeklyDigest: "Седмично резюме",
        weeklyDigestDesc: "Обобщение на активните ти процедури всеки понеделник.",
        productAnnouncements: "Продуктови съобщения",
        productAnnouncementsDesc: "Нови функции, обновени модели и важни новини.",
      },
      privacy: {
        exportTitle: "Експорт на данни",
        exportDesc: "Свали всичко което FormWise пази за теб на това устройство като JSON файл.",
        exportButton: "Свали моите данни",
        exportSuccess: "Данните са експортирани",
        conversationTitle: "Поверителност на разговорите",
        conversationDesc: "Контролирай дали FormWise да пази историята на разговорите ти.",
        defaultTemporary: "По подразбиране Временен чат",
        defaultTemporaryDesc: "Когато е включено, новите разговори не се пазят в историята по подразбиране. Можеш да включваш/изключваш и за конкретен разговор от Ask страницата.",
        redactPii: "Маскирай PII преди изпращане към модела",
        redactPiiDesc: "Авто-маскиране на ЕГН, телефон, IBAN и имейл преди да напуснат браузъра ти. Намалява експозицията към AI доставчика.",
        telemetryTitle: "Телеметрия",
        telemetryDesc: "Анонимна употреба за подобряване. Не се изпращат prompts или отговори — само имена на събития и времена.",
        telemetryToggle: "Разреши анонимна аналитика",
        telemetryToggleDesc: "Изпраща анонимизирани събития (напр. „зададен въпрос“). Никога не включва текст или лична информация.",
        clearTitle: "Изтриване на история",
        clearDesc: "Премахва всички запазени въпроси, процеси и активност от това устройство. Не може да се отмени.",
        clearButton: "Изтрий всичко",
        clearConfirm: "Да, изтрий всичко",
        clearedToast: "Историята е изтрита от това устройство",
      },
    },
  },
  de: {
    language: {
      label: "Sprache",
      change: "Sprache wechseln",
      english: "Englisch",
      bulgarian: "Bulgarisch",
      german: "Deutsch",
    },
    nav: {
      dashboard: "Dashboard",
      ask: "Fragen",
      browse: "Durchsuchen",
      history: "Verlauf",
    },
    common: {
      loading: "Laden...",
      close: "Schließen",
      clear: "Leeren",
      search: "Suchen",
      email: "E-Mail",
      password: "Passwort",
      fullName: "Vollständiger Name",
      noResults: "Keine Ergebnisse gefunden",
      tryAgain: "Erneut versuchen",
      getStarted: "Loslegen",
      createAccount: "Konto erstellen",
      logIn: "Anmelden",
      signUp: "Registrieren",
      viewAll: "Alle anzeigen",
      all: "Alle",
      questions: "Fragen",
      documents: "Dokumente",
      completed: "Abgeschlossen",
      inProgress: "In Bearbeitung",
      waiting: "Wartend",
      submitted: "Eingereicht",
      pending: "Ausstehend",
      missing: "Fehlend",
    },
    landing: {
      trustBadges: {
        secure: "Sicher & privat",
        instant: "Sofortanalyse",
        users: "10k+ Nutzer",
      },
      hero: {
        badge: "KI-gestützte Hilfe für jeden Prozess",
        titlePrefix: "Bürokratie bewältigen",
        titleHighlight: "ohne Kopfschmerzen",
        subtitle:
          "Lade Dokumente hoch, stelle Fragen und erhalte Schritt-für-Schritt-Hilfe für jeden Verwaltungsvorgang. Von Visa bis zu Gewerbegenehmigungen.",
        tryFree: "Kostenlos testen",
        seeHow: "So funktioniert's",
        trust: "Keine Kreditkarte erforderlich. 1 kostenlose Analyse zum Start.",
        docLabel: "Dokument",
        docValue: "Analysiert",
        stepLabel: "Schritt 3 von 5",
        stepValue: "Abgeschlossen",
      },
      demo: {
        title: "Jetzt ausprobieren",
        subtitle:
          "Stelle eine Frage zu Verwaltungsverfahren und sieh, wie FormWise helfen kann.",
        placeholder: "Frage zu einem Verwaltungsverfahren stellen...",
        tryLabel: "Versuche:",
        ctaText:
          "Willst du vollen Zugriff? Erstelle ein kostenloses Konto und verfolge deine Prozesse.",
        ctaButton: "Konto erstellen",
        samples: {
          residence: "Wie beantrage ich eine Aufenthaltserlaubnis in Bulgarien?",
          visa: "Welche Unterlagen brauche ich für ein Arbeitsvisum?",
          company: "Wie registriere ich ein Unternehmen im Ausland?",
        },
      },
      features: {
        title: "Alles, was du brauchst, um Papierkram zu meistern",
        subtitle:
          "Leistungsstarke Funktionen, die bürokratische Prozesse einfacher und stressfreier machen.",
        items: {
          analysisTitle: "Dokumentenanalyse",
          analysisDescription:
            "Lade ein Dokument hoch und erhalte sofort Einblicke. Unsere KI versteht Formulare, Anträge und amtliche Schreiben.",
          guidanceTitle: "Schritt-für-Schritt-Anleitung",
          guidanceDescription:
            "Erhalte klare, nummerierte Schritte für jedes Verfahren. So weißt du genau, was als Nächstes zu tun ist.",
          supportTitle: "Unterstützung für mehrere Länder",
          supportDescription:
            "Ob im Ausland oder zu Hause: Du erhältst lokalisierte Hilfe für Verfahren in verschiedenen Ländern.",
          timeTitle: "Zeitabschätzungen",
          timeDescription:
            "Sieh, wie lange jeder Schritt dauert. Plane besser und vermeide unnötige Wege oder Verzögerungen.",
          checklistTitle: "Dokumenten-Checkliste",
          checklistDescription:
            "Interaktive Checklisten helfen dir, alles rechtzeitig vorzubereiten und deinen Fortschritt zu verfolgen.",
          askTitle: "Frag alles",
          askDescription:
            "Du hast eine konkrete Frage? Frag einfach. Du bekommst schnelle und präzise Antworten.",
        },
      },
      authPrompt: {
        title: "Kostenloses Konto erstellen",
        body:
          "Du hast deine kostenlose Testanfrage genutzt. Erstelle ein Konto, um FormWise weiter zu nutzen und deine bürokratischen Prozesse zu verfolgen.",
        later: "Später",
        create: "Konto erstellen",
        existing: "Du hast bereits ein Konto? Anmelden",
      },
      footer: {
        product: "FormWise - KI-gestützte Bürokratie-Navigation",
        tagline: "Entwickelt für Menschen in komplexen Verwaltungsprozessen",
      },
      loading: "Deine Frage wird analysiert...",
    },
    header: {
      dashboard: "Dashboard",
      login: "Anmelden",
      getStarted: "Loslegen",
    },
    appShell: {
      openMenu: "Menü öffnen",
      closeMenu: "Menü schließen",
      openCommandPalette: "Befehlspalette öffnen",
      searchPlaceholder: "Suchen oder fragen...",
      searchFull: "Verfahren suchen, KI fragen...",
      collapse: "Einklappen",
      expand: "Seitenleiste erweitern",
      settings: "Einstellungen",
      logout: "Abmelden",
      logoAlt: "FormWise Wisp Logo",
    },
    askInput: {
      country: "Land:",
      selectCountryHelp: "Wähle das Land für dieses Verfahren",
      followUpMode: "Nachfrage-Modus",
      placeholder: "Frage nach Verwaltungsverfahren in {country}...",
      attach: "Dokument anhängen",
      attachHelp: "PDF oder Bilder bis 10MB",
      analyzing: "Analysiere...",
      ask: "Fragen",
      askFollowUp: "Nachfrage stellen",
      followUpTip:
        "Tipp: Du kannst nach konkreten Schritten, Dokumenten, Kosten oder anderen Details aus der vorherigen Antwort fragen. Der bisherige Kontext wird automatisch einbezogen.",
      dropHere: "Dokument hier ablegen",
      selectCountry: "Land auswählen",
    },
    askPage: {
      title: "FormWise fragen",
      subtitle:
        "Stelle Fragen zu Verwaltungsverfahren und erhalte detaillierte, quellenbasierte Hilfe.",
      hideHistory: "Verlauf ausblenden",
      showHistory: "Verlauf anzeigen",
      historyTitle: "Gesprächsverlauf",
      you: "Du",
      searchingSources: "Offizielle Quellen werden durchsucht...",
      compiling: "FormWise erstellt detaillierte Hilfe mit rechtlichen Verweisen",
      suggestionsLabel: "Zum Beispiel:",
      detailPrompt: "Bitte gib mehr Details an",
      moreInfoBody:
        "Für {country} brauchen wir genauere Angaben, um dir besser helfen zu können.",
      needSpecificInfo:
        "Wir brauchen genauere Informationen zu deiner Frage für {country}.",
      missingSpecificInfo:
        "Wir konnten keine ausreichend konkreten Informationen für {country} finden. Versuche eine spezifischere Frage oder prüfe das richtige Land.",
      failedAnalyze: "Die Analyse deiner Frage ist fehlgeschlagen. Bitte versuche es erneut.",
      failedAnswer: "Die Antwort konnte nicht geladen werden. Bitte versuche es erneut.",
      checkConnection: "Prüfe deine Verbindung und versuche es erneut.",
      analysisComplete: "Analyse abgeschlossen",
      analysisDoneFor: "Dokument für den Zuständigkeitsbereich {country} analysiert.",
      answerReady: "Antwort bereit",
      procedureInfoFor: "Verfahrensinformationen für {country}.",
      searchingFor: "Suche für {country}...",
      providingGuidance: "FormWise erstellt detaillierte Hinweise.",
      detailedReady: "Detaillierte Antwort bereit",
      detailedInfoFor: "Informationen für {country}: {procedure}",
      selectCountry: "Bitte wähle ein Land",
      selectCountryDescription: "Das hilft uns, genaue Informationen zu liefern.",
      selectCountryFirst: "Bitte zuerst ein Land auswählen",
      loadingFor: "Analyse für {country}...",
      loadingDescription:
        "FormWise durchsucht die Wissensdatenbank und offizielle Quellen.",
      viewInChat: "Im Chat anzeigen",
      newQuestion: "Neue Frage",
      followUpTitle: "Nachfragen",
      followUpSubtitle: "Stelle eine Nachfrage für genauere Details",
      lastUpdated: "Zuletzt aktualisiert: {date}",
      suggestions: {
        s1: "Wie bekomme ich eine Aufenthaltserlaubnis?",
        s2: "Welche Unterlagen brauche ich für eine Unternehmensregistrierung?",
        s3: "Wie beantrage ich ein Arbeitsvisum?",
        s4: "Wie läuft die Passverlängerung ab?",
      },
      followUps: {
        requirements: "Was sind die genauen Voraussetzungen?",
        timeline: "Wie lange dauert das?",
        cost: "Wie hoch sind die Gesamtkosten?",
        location: "Wo genau muss ich hingehen?",
        visaWork: "Darf ich mit diesem Visum arbeiten?",
        visaFamily: "Kann ich meine Familie mitbringen?",
        visaRejection: "Was passiert bei einer Ablehnung?",
        permitExtend: "Kann ich diese Erlaubnis verlängern?",
        permitRenewal: "Welche Voraussetzungen gelten für die Verlängerung?",
        permitStatus: "Kann ich meinen Status ändern?",
        catRequirements: "Voraussetzungen",
        catTimeline: "Dauer",
        catCosts: "Kosten",
        catLocation: "Ort",
        catWorkRights: "Arbeitsrechte",
        catFamily: "Familie",
        catRejection: "Ablehnung",
        catExtension: "Verlängerung",
        catRenewal: "Erneuerung",
        catStatusChange: "Statuswechsel",
      },
      moreInfoTips: {
        intro: "Füge zum Beispiel diese Details hinzu:",
        one: 'Die genaue Art des Verfahrens (z. B. "Arbeitserlaubnis" statt nur "Arbeit")',
        two: "Deine Staatsangehörigkeit oder deinen aktuellen Status",
        three: "Falls relevant: konkrete Stadt oder Region",
      },
      responseReceived: "Antwort erhalten",
      trackProcess: "Prozess verfolgen",
      trackedProcess: "Verfolgt",
      trackingDisabledTemporary: "Temporäre Chats können keine verfolgten Prozesse erstellen.",
      processTracked: "Prozess wird verfolgt",
      validation: {
        questionTooShort: "Die Frage ist zu kurz. Bitte gib mehr Details an.",
        addMoreContext: "Füge mehr Kontext zu deiner Situation hinzu",
        includeProcedureType: "Nenne die Art des Verfahrens (z. B. Visum, Genehmigung, Bescheinigung)",
        noCountrySelected: "Kein Land ausgewählt. Bitte wähle ein Land.",
        selectCountryForProcedure: "Wähle das Land aus, in dem du dieses Verfahren brauchst",
        vagueTerms: "Die Frage enthält unklare Begriffe. Bitte sei genauer.",
        replaceVagueWords: "Ersetze unklare Wörter durch konkrete Details",
        missingQuestionMark: "Bitte beende deine Frage mit einem Fragezeichen (?).",
        phraseClearly: "Bitte formuliere deine Frage klar.",
        startWithQuestionWord: "Beginne mit „Wie“, „Was“, „Wo“, „Wann“ oder „Kann“",
        needsMoreDetailsTitle: "Die Frage braucht mehr Details",
        pleaseBeMoreSpecific: "Bitte sei genauer",
      },
      temporaryChatShort: "Temporär",
      continuingFromHistory: "Aus Verlauf fortsetzen",
    },
    answerDisplay: {
      verified: "Verifiziert",
      verifiedLabel: "Verifiziert:",
      legalFoundation: "Rechtsgrundlage",
      law: "Gesetz:",
      article: "Artikel:",
      year: "Jahr:",
      source: "Quelle:",
      officialSource: "Offizielle Quelle",
      scopeTitle: "Umfang dieses Verfahrens",
      scopeCovers: "Dieses Verfahren deckt ab",
      scopeDoesNotCover: "Deckt NICHT ab",
      eligibility: "Voraussetzungen",
      whoCanApply: "Wer kann beantragen",
      prerequisites: "Vorbedingungen",
      exceptions: "Ausnahmen",
      whoShouldNotApply: "Wer nicht beantragen sollte",
      timeline: "Bearbeitungsdauer",
      minimumTime: "Mindestdauer",
      maximumTime: "Maximaldauer",
      varies: "Variiert",
      expeditedAvailable: "Beschleunigte Option verfügbar",
      time: "Zeit",
      contactOffice: "Amt kontaktieren",
      additionalCost: "Zusätzliche Kosten",
      timelineFactors: "Faktoren, die die Bearbeitung verlängern können",
      afterApproval: "Nach der Genehmigung:",
      costBreakdown: "Kostenübersicht",
      governmentFees: "Behördliche Gebühren",
      translationCosts: "Übersetzungskosten",
      notarizationCosts: "Notarkosten",
      otherCosts: "Weitere Kosten",
      totalEstimatedCost: "Geschätzte Gesamtkosten",
      paymentMethods: "Akzeptierte Zahlungsmethoden:",
      stepsToComplete: "Schritte zur Erledigung",
      completedCount: "{completed}/{total} erledigt",
      potentialIssues: "Mögliche Probleme:",
      requiredDocuments: "Erforderliche Dokumente",
      readyCount: "{ready}/{total} bereit",
      required: "Erforderlich",
      optional: "Optional",
      translationRequired: "Übersetzung erforderlich",
      whereToGet: "Wo erhältlich:",
      validFor: "Gültig für:",
      cost: "Kosten:",
      legalBasis: "Rechtsgrundlage:",
      requirements: "Anforderungen:",
      detailedInstructions: "Detaillierte Anleitung",
      steps: "Schritte",
      fullResponse: "Vollständige Antwort",
      noDetailedInstructions: "Keine detaillierte Anleitung verfügbar.",
      whereToGo: "Wohin gehen",
      appointmentRequired: "Termin erforderlich",
      visitWebsite: "Website besuchen",
      viewOnMap: "Auf Karte anzeigen",
      importantWarnings: "Wichtige Warnungen",
      commonRejections: "Häufige Ablehnungsgründe",
      howToAvoid: "So vermeiden:",
      scamsToAvoid: "Betrug vermeiden",
      whatNotToDo: "Was NICHT tun",
      additionalNotes: "Zusätzliche Hinweise",
      relatedProcedures: "Verwandte Verfahren",
      doFirst: "Zuerst erledigen",
      doNext: "Als Nächstes",
      alternative: "Alternative",
    },
    processes: {
      title: "Laufende Prozesse",
      empty: "Keine laufenden Prozesse",
      startInquiry: "Neue Anfrage starten",
      next: "Nächster Schritt:",
      progress: "Fortschritt",
      percentComplete: "{percent}% abgeschlossen",
      overallProgress: "Gesamtfortschritt",
      nextDeadline: "Nächste Frist",
      estimatedCompletion: "Vorauss. Abschluss",
      location: "Ort",
      notes: "Notizen",
      notesPlaceholder: "Private Notizen zu diesem Prozess hinzufügen...",
      steps: "Prozessschritte",
      current: "Aktuell",
      upcoming: "Bevorstehend",
      documents: "Erforderliche Dokumente",
      newDocumentPlaceholder: "Dokument oder Checklisteneintrag hinzufügen...",
      addDocument: "Dokument hinzufügen",
      askQuestion: "Frage stellen",
    },
    auth: {
      login: {
        title: "Willkommen zurück",
        subtitle: "Melde dich an, um fortzufahren",
        forgot: "Passwort vergessen?",
        passwordPlaceholder: "Passwort eingeben",
        loggingIn: "Anmeldung läuft...",
        orContinue: "Oder fortfahren mit",
        socialSoon: "Social Login kommt bald",
        noAccount: "Noch kein Konto?",
        failed: "Anmeldung fehlgeschlagen",
      },
      register: {
        title: "Konto erstellen",
        subtitle: "Bürokratie mit KI leichter bewältigen",
        namePlaceholder: "Max Mustermann",
        passwordPlaceholder: "Mindestens 6 Zeichen",
        creating: "Konto wird erstellt...",
        benefitsTitle: "Das bekommst du:",
        already: "Du hast bereits ein Konto?",
        failed: "Registrierung fehlgeschlagen",
        passwordTooShort: "Das Passwort muss mindestens 6 Zeichen lang sein",
        features: {
          one: "Unbegrenzte Dokumentenanalyse",
          two: "Dashboard zur Prozessverfolgung",
          three: "Dokumentenverlauf",
          four: "Priorisierter Support",
        },
      },
    },
    browse: {
      title: "Verfahren durchsuchen",
      subtitle: "Entdecke verfügbare Verwaltungsverfahren nach Kategorie.",
      askInstead: "Stattdessen KI fragen",
      bannerTitle: "Weitere Länder folgen bald",
      bannerBody:
        "Wir erweitern die Abdeckung auf weitere Länder. Der aktuelle Fokus liegt auf Bulgarien.",
      prompt: "Nicht gefunden, was du brauchst?",
      askAi: "KI fragen",
      loading: "Laden...",
    },
    history: {
      title: "Verlauf",
      subtitle: "Durchsuche frühere Fragen und Dokumentenanalysen.",
      searchPlaceholder: "Verlauf durchsuchen...",
      noResultsBody: "Passe deine Suche oder Filter an.",
      askNew: "Neue Frage stellen",
    },
    dashboard: {
      badge: "FormWise · KI-Assistent für europäische Bürokratie",
      morning: "Guten Morgen",
      afternoon: "Guten Tag",
      evening: "Guten Abend",
      friend: "Freund",
      summary:
        "Hier ist ein Überblick über deinen bürokratischen Weg. Drücke jederzeit {shortcut}, um zu suchen, zu navigieren oder die KI zu fragen.",
      newQuestion: "Neue Frage",
      openPalette: "oder Befehlspalette öffnen →",
      welcomeTitle: "Willkommen bei FormWise!",
      welcomeBody:
        "Frage unseren KI-Assistenten zu jedem bürokratischen Prozess und erhalte Hilfe Schritt für Schritt.",
      quickActions: "Schnellaktionen",
      welcomeAction: "Loslegen",
      todayPriorities: "Heutige Prioritäten",
      planWithAi: "Mit KI planen",
      noPriorities: "Keine Prioritäten gesetzt",
      getStartedWithAi: "Mit KI starten",
      priorityCritical: "Kritisch",
      prioritySoon: "Bald",
      priorityUpcoming: "Bevorstehend",
      due: "fällig",
      left: "übrig",
    },
    clippy: {
      assistantLabel: "FormWise-Assistent",
      nextTip: "Nächster Tipp",
      hide: "Für diese Sitzung ausblenden",
      restore: "FormWise-Helfer wiederherstellen",
      dragHint: "Zieh mich irgendwohin",
      resetPosition: "Position zurücksetzen",
      tips: {
        default: {
          one: "Brauchst du Hilfe im Bürokratie-Labyrinth? Ich zeige dir den passenden Ablauf.",
          two: "Lade Dokumente hoch, wenn der genaue Wortlaut wichtig ist. Formulare lieben winzige Details.",
          three: "Wenn eine Antwort zu allgemein wirkt, stelle eine Nachfrage mit Status, Stadt und Frist.",
        },
        dashboard: {
          one: "Dein Dashboard zeigt jetzt nur noch deine eigenen gespeicherten Fragen und verfolgten Prozesse.",
          two: "Nutze „Prozess verfolgen“ nach einer Antwort, um Hinweise in eine laufende Checkliste zu verwandeln.",
          three: "Leere Zustände sind hier gut: Sie bedeuten, dass keine erfundenen Papiermonster deine Daten heimsuchen.",
        },
        ask: {
          one: "Temporärer Chat hält Experimente aus Verlauf und Dashboard heraus.",
          two: "Wenn FormWise nach fehlendem Kontext fragt, antworte im Panel und der volle Thread wird fortgesetzt.",
          three: "Starke Prompts enthalten Land, Status, Frist und den genauen Namen des Verfahrens.",
        },
        history: {
          one: "Öffne einen Verlaufseintrag, um den gespeicherten Prompt, die Antwort, die Konfidenz und die nächsten Schritte zu sehen.",
          two: "„Continue in Ask“ stellt die gespeicherte Antwort wieder als Gesprächskontext her.",
          three: "Der Verlauf ist derzeit lokal in diesem Browser, also löscht das Leeren des Browser-Speichers ihn ebenfalls.",
        },
      },
    },
    chatNoSpecificInfo:
      "Ich habe noch keine konkreten Informationen zu diesem Verfahren. Bitte prüfe die offizielle Regierungswebsite für {country}.",
    compareInsufficientContext:
      "Ich habe noch nicht genug offiziellen Quellenkontext, um diese Länder zuverlässig zu vergleichen. Bitte präzisiere die Frage oder prüfe offizielle Regierungswebsites.",
    journeyLimitedInfo:
      "Derzeit sind nur begrenzte offizielle Verfahrensinformationen für {country} verfügbar.",
    journeyMissingCoverage: "Fehlende belastbare Abdeckung für: {areas}.",
    journeyVerifyRequirements:
      "Prüfe Visum-, Melde- und Genehmigungsanforderungen auf offiziellen Regierungswebsites, bevor du handelst.",
    compareExcludedCountry:
      "{code} wurde ausgeschlossen, weil die offizielle Quellenlage für einen verlässlichen Vergleich zu schwach war.",
    compareMissingStructuredCountry:
      "{code} hatte Quellenabdeckung, lieferte aber keinen ausreichend belastbaren strukturierten Vergleichseintrag.",
    compareNoGroundedOutput:
      "Ich habe nicht genug belastbare länderspezifische Ausgaben, um diese Optionen verlässlich zu vergleichen.",
    journeyMissingPhases:
      "Einige erwartete Roadmap-Phasen fehlten in der Modellausgabe und wurden als leere Platzhalter wieder eingefügt: {phases}.",
    journeyLimitedCoverage:
      "Einige Teile dieses Plans beruhen auf begrenzter offizieller Quellenabdeckung. Prüfe diese Bereiche in offiziellen Quellen: {areas}.",
    journeyNoStrongGrounding:
      "Keine Bereiche der Umzugsplanung waren stark im offiziellen Quellenkontext verankert.",
    settings: {
      title: "Einstellungen",
      subtitle: "Konfiguriere AI-Anbieter, Konto, Benachrichtigungen und Datenschutz.",
      tabs: {
        ai: "AI-Modelle",
        billing: "Abrechnung",
        account: "Konto",
        notifications: "Benachrichtigungen",
        privacy: "Datenschutz",
        developer: "Entwickler",
      },
      common: {
        save: "Speichern",
        saving: "Speichert...",
        reset: "Auf Standard zurücksetzen",
        loading: "Lädt...",
      },
      ai: {
        defaultProvider: "Standard-AI-Anbieter",
        defaultProviderDesc: "Wähle, welcher Anbieter deine Fragen beantwortet. Standard ist",
        envDefaultSuffix: ". Diese Auswahl überschreibt das für dein Konto.",
        provider: "Anbieter",
        model: "Modell",
        notConfigured: "(nicht konfiguriert)",
        notConfiguredTitle: "Anbieter nicht konfiguriert",
        notConfiguredHint: "Setze die entsprechende Umgebungsvariable in",
        params: "Generierungsparameter",
        paramsDesc: "Sampling-Feintuning. Standardmäßig anbieterspezifische Werte.",
        temperature: "Temperatur",
        temperatureHint: "Niedriger = deterministisch. Höher = kreativ. Empfohlen: 0.2–0.7 für rechtliche Genauigkeit.",
        maxTokens: "Max. Tokens",
        maxTokensPlaceholder: "Anbieter-Standard (meist 4096)",
        systemPromptOverride: "System-Prompt überschreiben",
        systemPromptPlaceholder: "Leer lassen für den eingebauten FormWise-Prompt.",
        systemPromptHint: "Erweitert: Ersetzt den Standard-Prompt. Leer lassen für den Standard.",
        fallbackChain: "Fallback-Kette",
        fallbackChainDesc: "Wenn der primäre Anbieter ausfällt, geht der Router diese Liste durch. Konfigurierte Anbieter zuerst.",
        providerStatus: "Anbieterstatus",
        providerStatusDesc: "Live-Konfigurationsprüfung. Klicke „Test“ für einen Ping-Prompt.",
        defaultLabel: "Standard",
        testButton: "Test",
        testFailed: "fehlgeschlagen",
        savedToast: "Einstellungen gespeichert",
        resetToast: "Auf Umgebungs-Defaults zurückgesetzt",
      },
      billing: {
        currentPlan: "Aktueller Plan",
        period: "Zeitraum",
        renews: "Verlängert sich",
        manage: "Abonnement verwalten",
        questions: "Fragen (/api/ask)",
        documentAnalyses: "Dokumentanalysen",
        relocationPlans: "Umzugspläne",
        countryComparisons: "Ländervergleiche",
        unlimited: "unbegrenzt",
        recommended: "Empfohlen",
        currentPlanLabel: "Aktueller Plan",
        cancelViaPortal: "Über Portal kündigen",
        free: "Kostenlos",
        upgradeToPro: "Auf Pro upgraden",
        upgradeToBusiness: "Auf Business upgraden",
        perMonth: "/ Monat",
        billingFooter: "Abrechnung über LemonSqueezy — MwSt. und EU-Compliance inklusive. Du kannst jederzeit über „Abonnement verwalten“ kündigen.",
      },
      account: {
        profile: "Profil",
        profileDesc: "Kontodaten, die in deinem Dashboard angezeigt werden.",
        displayName: "Anzeigename",
        email: "E-Mail",
        savePhase2: "Speichern (Phase 2)",
        password: "Passwort",
        passwordDesc: "Phase 1 speichert in localStorage. Echte Passwortänderung benötigt Backend-Auth aus Phase 2.",
        signOut: "Abmelden",
        signOutDesc: "Sitzung auf diesem Gerät beenden.",
        signOutButton: "Abmelden",
        deleteTitle: "Konto löschen",
        deleteDesc: "Konto und alle lokal gespeicherten Daten endgültig entfernen. Nicht rückgängig.",
        deleteButton: "Mein Konto löschen",
        deleteConfirm: "Ja, endgültig löschen",
        cancel: "Abbrechen",
      },
      notifications: {
        title: "Benachrichtigungseinstellungen",
        desc: "Steuere, wie und wann FormWise dich kontaktiert. E-Mail-Benachrichtigungen werden mit der Resend-Integration aus Phase 3 aktiviert.",
        inAppToasts: "App-Benachrichtigungen",
        inAppToastsDesc: "Statusupdates und Bestätigungen während der Nutzung.",
        emailReminders: "Schritt-Erinnerungen",
        emailRemindersDesc: "E-Mail-Erinnerungen vor anstehenden Verfahrensschritten.",
        emailDeadlines: "Fristerinnerung",
        emailDeadlinesDesc: "E-Mail-Warnungen 7 / 3 / 1 Tag vor gesetzlichen Fristen.",
        govSourceUpdates: "Aktualisierungen offizieller Quellen",
        govSourceUpdatesDesc: "Benachrichtigung bei Änderung einer offiziellen Seite zu deinem Verfahren.",
        weeklyDigest: "Wöchentliche Zusammenfassung",
        weeklyDigestDesc: "Zusammenfassung deiner aktiven Verfahren jeden Montag.",
        productAnnouncements: "Produktankündigungen",
        productAnnouncementsDesc: "Neue Funktionen, Modell-Upgrades und wichtige Hinweise.",
      },
      privacy: {
        exportTitle: "Datenexport",
        exportDesc: "Lade alles herunter, was FormWise auf diesem Gerät über dich speichert. Der Export ist eine JSON-Datei.",
        exportButton: "Meine Daten exportieren",
        exportSuccess: "Daten exportiert",
        conversationTitle: "Gesprächsdatenschutz",
        conversationDesc: "Steuere, ob FormWise deine Gesprächshistorie speichert.",
        defaultTemporary: "Temporären Chat als Standard",
        defaultTemporaryDesc: "Wenn aktiviert, werden neue Gespräche standardmäßig nicht gespeichert. Du kannst weiterhin pro Gespräch umschalten.",
        redactPii: "PII vor dem Senden ans Modell maskieren",
        redactPiiDesc: "Auto-Maskierung von EGN, Telefon, IBAN und E-Mail in deinen Nachrichten, bevor sie den Browser verlassen.",
        telemetryTitle: "Telemetrie",
        telemetryDesc: "Anonyme Nutzungsanalyse zur Verbesserung. Keine Prompts oder Antworten — nur Ereignisnamen und Zeitangaben.",
        telemetryToggle: "Anonyme Analyse erlauben",
        telemetryToggleDesc: "Sendet anonymisierte Ereignisse (z.B. „Frage gestellt“). Niemals Prompt-Text oder persönliche Informationen.",
        clearTitle: "Gesprächshistorie löschen",
        clearDesc: "Entfernt alle gespeicherten Fragen, Prozesse und Aktivitäten von diesem Gerät. Nicht rückgängig.",
        clearButton: "Gesamte Historie löschen",
        clearConfirm: "Ja, alles löschen",
        clearedToast: "Historie auf diesem Gerät gelöscht",
      },
    },
  },
};

function getTranslationValue(language: Language, key: string): TranslationValue | undefined {
  return key.split(".").reduce<TranslationValue | undefined>((current, part) => {
    if (!current || typeof current === "string") {
      return undefined;
    }

    return current[part];
  }, TRANSLATIONS[language]);
}

function interpolate(template: string, params?: Record<string, string>): string {
  if (!params) {
    return template;
  }

  return template.replace(/\{(\w+)\}/g, (_, token: string) => params[token] ?? `{${token}}`);
}

export function isLanguage(value: string | undefined): value is Language {
  return value === "bg" || value === "en" || value === "de";
}

export function getLanguageName(language?: string): string {
  return isLanguage(language) ? LANGUAGE_NAMES[language] : LANGUAGE_NAMES[DEFAULT_LANGUAGE];
}

export function t(
  language: string | undefined,
  key: string,
  params?: Record<string, string>,
): string {
  const safeLanguage = isLanguage(language) ? language : DEFAULT_LANGUAGE;
  const translated = getTranslationValue(safeLanguage, key) ?? getTranslationValue(DEFAULT_LANGUAGE, key);

  if (typeof translated !== "string") {
    return key;
  }

  return interpolate(translated, params);
}
